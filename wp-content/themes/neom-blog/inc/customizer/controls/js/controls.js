		...
		color: {
			...
			getTemplate: function ( data ) {
				...
				html +=
					...
					'" value="' +
					data.value +
					'" class="neom-color-control" data-id="' +
					data['data-id'] +
					'"/>';

				return (
					'<div class="neom-input-container" data-id="' +
					data.id +
					'">' +
					html +
					'</div>'
			...
			init: function ( control ) {
				var picker = jQuery(
					'.neom-color-control[data-id="' + control.id + '"]'
				    ),
				    clear;
...
(
	function () {
		 ...
		 */
		wp.customize.neomDynamicControl = wp.customize.Control.extend( {
			...
			ready: function () {
				...
				control.deferred.embedded.done( function () {
					control.initneomControl( control );
				} );
			},
			...
			embed: function () {
				...
				wp.customize.section( sectionId, function ( section ) {
					if (
						'neom-expanded' === section.params.type ||
						section.expanded() ||
			 ...
			 * @returns {void}
			 */
			initneomControl: function ( control ) {
				if ( 'undefined' !== typeof neom.control[control.params.type] ) {
					neom.control[control.params.type].init( control );
					return;
				}
			...
			neomValidateCSSValue: function ( value ) {
				var validUnits = [
					    'rem',
					    'em',
					    'ex',
...
_.each( neom.control, function ( obj, type ) {
	wp.customize.controlConstructor[
		type
		] = wp.customize.neomDynamicControl.extend( {} );
} );
 ...
 * Control: Slider.
 */
wp.customize.controlConstructor['neom-slider'] = wp.customize.Control.extend( {
	...
	ready: function () {
		...
		var control      = this,
		    ...
		    slider       = control.container.find( '.neom-slider' ),
		...
		// Init the control.
		if ( !_.isUndefined( window.neomControlLoader ) && _.isFunction( neomControlLoader ) ) {
			neomControlLoader( control );
		} else {
			control.initneomControl();
		}
	},

	initneomControl: function () {
 ...
 * Control: Text.
 */
wp.customize.controlConstructor['neom-text'] = wp.customize.Control.extend( {
 ...
 * Control: Sortable.
 */
/* global neomControlLoader */
wp.customize.controlConstructor['neom-sortable'] = wp.customize.Control.extend( {
	...
	ready: function () {
		...
		// Init the control.
		if (
			!_.isUndefined( window.neomControlLoader ) &&
			_.isFunction( neomControlLoader )
		) {
			neomControlLoader( control );
		} else {
			control.initneomControl();
	...
	initneomControl: function () {
		...
		var control = this;
		control.container.find( '.neom-controls-loading-spinner' ).hide();
 ...
 * Control: Radio image.
 */
wp.customize.controlConstructor['neom-radio-image'] = wp.customize.neomDynamicControl.extend( {} );

/**
 * Control: Toggle.
 */
wp.customize.controlConstructor['neom-toggle'] = wp.customize.neomDynamicControl.extend( {
 ...
 */
wp.customize.controlConstructor['neom-radio-buttonset'] = wp.customize.Control.extend( {} );

/**
 * Control: Dimensions.
 */
;
/* global dimensionsneomL10n */
/* global neomControlLoader */
wp.customize.controlConstructor['neom-dimensions'] = wp.customize.neomDynamicControl.extend( {
			...
			setting.bind( function ( value ) {
				...
				_.each( ['top', 'right', 'bottom', 'left'], function ( direction ) {
					if ( !_.isUndefined( value[direction] ) ) {
						if ( false === control.neomValidateCSSValue( value[direction] ) ) {
							subs[direction] = dimensionsneomL10n[direction];
						} else {
				...
				if ( !_.isEmpty( subs ) ) {
					message = dimensionsneomL10n['invalid-value'] + ' (' + _.values( subs ).toString() + ') ';
...
;
/* global neomControlLoader */
wp.customize.controlConstructor['neom-background'] = wp.customize.Control.extend( {
	...
	initneomControl: function () {
		...
		var control = this,
		    value   = control.setting._value,
		    picker  = control.container.find( '.neom-color-control' );
 ...
 * Control: Editor.
 */
wp.customize.controlConstructor['neom-editor'] = wp.customize.neomDynamicControl.extend( {
